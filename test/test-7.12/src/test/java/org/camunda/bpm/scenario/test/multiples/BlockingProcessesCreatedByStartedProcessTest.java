 ...
 */
public class BlockingProcessesCreatedByStartedProcessTest {
  ...
  public void defaults() {
    ...
    when(otherProcess.waitsAtUserTask("UserTaskOther")).thenReturn(task -> task.complete());
    when(otherProcess.runsProcessInstance("AnotherProcess")).thenReturn(use(anotherProcess));
    when(anotherProcess.waitsAtUserTask("UserTaskAnother")).thenReturn(task -> task.complete());

  }

  @Test
  @Deployment(resources = {"org/camunda/bpm/scenario/test/multiples/BlockingProcessesCreatedByStartedProcess.bpmn"})
